...
(function(){ 
    ...
    var LIGNTNINGS_PARTICLE_URL = "http://metaverse.bashora.com/scripts/hytrion_cloud/???????.png";
    var THUNDER_SOUND = "http://metaverse.bashora.com/scripts/hytrion_cloud/?????.mp3";
    var thunderSound, thunderInjector; 
    ...
    this.preload = function(entityID) {
        ...
        //############## NOCTURN LIGHTNINGS AND THUNDER #############
        thunderSound = SoundCache.getSound(THUNDER_SOUND);
        //############## END NOCTURN LIGHTNINGS AND THUNDER #############        
    };
    ...
    function updateNavigation() {
        if (!isInitiated){
            ...
            var hytrionCurrentHour = (GetCurrentCycleValue(8640000, DAY_DURATION, timestamp)/100) / 3600;
            
            if ( hytrionCurrentHour > 21.5 || hytrionCurrentHour < 2.5 ) {
                if (storming) {
                    // Manage thunder and color
                    Entities.editEntity(lightningsID, { "position": myAvPos });
                    
                    if (Math.random() > 0.04) {
                        thunderInjector = Audio.playSound(thunderSound, {
                            "loop": false,
                            "localOnly": true,
                            "volume": Math.random();
                            });
